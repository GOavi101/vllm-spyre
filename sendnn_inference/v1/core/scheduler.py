...
class ChunkedPrefillSpyreScheduler(Scheduler):
    ...
    performed by the base scheduler is recorded as ``num_output_placeholders``
    on each prefill chunk; ``update_from_output()`` reconciles the advance
    against the runner's actual left-padding / prefix-cache report and
    decrements the placeholder. The same code path works for sync mode where
    placeholders are added and cleared within a single step (a no-op).
    ``update_from_output()`` also filters scheduler outputs to only include
    requests that were actually executed (relevant under run-ahead).
    """
    ...
    def _update_after_schedule(self, scheduler_output: SchedulerOutput) -> None:
        """Track in-flight prefill-chunk advances as ``num_output_placeholders``.
        ...
        pick the next chunk without waiting for the model output. We mirror that
        advance into ``num_output_placeholders`` so ``update_from_output`` can
        later reconcile the optimistic value against what actually committed
        (after left-padding / prefix-cache adjustments).

        Decode-side placeholders are managed by ``AsyncScheduler`` (which our
        async variant inherits via the MRO); here we only handle prefill
        chunks. Decode requests fall through unchanged.
        """
    ...
    def update_from_output(self, scheduler_output, model_runner_output):
        ...
        # by the delta, and clear the placeholder bump made in
        # ``_update_after_schedule``. This replaces the previous snapshot
        # deque mechanism: the per-chunk truth is derivable from the runner
        # output and ``scheduler_output.num_scheduled_tokens``.
        for req_id in executed_req_ids:
            req = self.requests.get(req_id)
            if req is None:
                continue
            scheduled_n = scheduler_output.num_scheduled_tokens.get(req_id, 0)
            if scheduled_n == 0 or req.num_output_placeholders == 0:
                # Not a prefill-chunk we tracked (e.g. decode, or already
                # reconciled by AsyncScheduler).
                continue

            # Optimistic position after this chunk's advance (already applied
            # by the base scheduler in _update_after_schedule).
            optimistic_after = req.num_computed_tokens
            committed_before = optimistic_after - scheduled_n
            is_first_chunk = committed_before == 0
            is_last_chunk = optimistic_after >= req.num_prompt_tokens
            ...
            if is_first_chunk and not is_last_chunk:
                ...
                adjusted = self.adjust_computed_tokens(
                    computed_tokens=optimistic_after,
                    left_padding=left_padding,
                    prefix_cache_len=prefix_cache_len,
                )
                req.num_computed_tokens += adjusted - optimistic_after

            # Clear the placeholder we added in _update_after_schedule.
            req.num_output_placeholders = max(
                0, req.num_output_placeholders - scheduled_n
            )
        ...
        self.ongoing_prefills = [
            ...
            if (req.num_computed_tokens - req.num_output_placeholders) < req.num_prompt_tokens
        ]
    ...
    def schedule(self) -> "SchedulerOutput":
        ...
        # ``num_output_placeholders`` records the in-flight portion; a request
        # is still prefilling iff its committed position is below the prompt
        # length. Using the optimistic position alone is also acceptable here
        # because the base scheduler will not schedule beyond the prompt, and
        # we only need to recognise that the request is mid-prefill.
        self.ongoing_prefills = [
            req
            for req in self.running
            if (req.num_computed_tokens - req.num_output_placeholders)
