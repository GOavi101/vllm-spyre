...
class SpyreVocabParallelEmbedding(VocabParallelEmbedding):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Compiles the Spyre embedding kernel and registers this instance in
        static_forward_context for retrieval during the custom op forward pass.
        """
    ...
    def forward_native(self, input_: torch.Tensor) -> torch.Tensor:
        """CPU execution via torch-spyre fallback.

        The Spyre device does not have a native embedding kernel — aten.embedding.default
        is handled by torch-spyre's spyre__embedding CPU fallback. Moving tensors to the
        Spyre device causes a DtException (unsupported data format). Therefore we run
        F.embedding directly on CPU, which is what the fallback does internally anyway.

        Padding to _SPYRE_MIN_SEQ_LEN is still applied for consistency with other
        Spyre ops, even though the embedding itself runs on CPU.
