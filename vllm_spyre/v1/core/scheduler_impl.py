...
"""
Spyre scheduler implementations using factory functions.

This module provides factory functions that create Spyre scheduler classes
with the appropriate base class (Scheduler or AsyncScheduler) at runtime,
...
def _create_pooling_scheduler(base_scheduler_cls):
    """Factory function to create PoolingSpyreScheduler with the appropriate base class.
    
    Args:
        base_scheduler_cls: Either Scheduler or AsyncScheduler from vLLM
    
    Returns:
        PoolingSpyreScheduler class with the specified base
    """
    class PoolingSpyreScheduler(base_scheduler_cls):  # type: ignore
        """Support of pooling models"""

        def __init__(self, *args, **kwargs) -> None:
            # Initialize base scheduler (vLLM Scheduler or AsyncScheduler)
            super().__init__(*args, **kwargs)
            # Initialize model config (needed by implementation)
        ...
        def schedule(self) -> SchedulerOutput:
            ...
            # Apply Spyre-specific warmup shape constraints.
            # AsyncScheduler has the same self.running/self.waiting as Scheduler,
            # so these constraints are compatible with both sync and async scheduling.
            ...
            # delegate to super of BaseScheduler: base V1 Scheduler
...
def _create_chunked_prefill_scheduler(base_scheduler_cls):
    """Factory function to create ChunkedPrefillSpyreScheduler with the appropriate base class.
    
    Args:
        base_scheduler_cls: Either Scheduler or AsyncScheduler from vLLM
    
    Returns:
        ChunkedPrefillSpyreScheduler class with the specified base
    """
    is_async = base_scheduler_cls.__name__ == 'AsyncScheduler'
    
    class ChunkedPrefillSpyreScheduler(base_scheduler_cls):  # type: ignore
        """
        ...
        def __init__(self, *args, **kwargs) -> None:
            ...
            # Track if we've already warned about async mode (to avoid log spam)
            self._async_warning_logged = False

        def update_from_output(self, scheduler_output, model_runner_output):
            # Handle empty model runner output (no execution in this step)
            # This can happen with async scheduling when scheduler runs ahead
            if not model_runner_output.req_ids:
                # No requests were executed. If async mode and scheduler_output has requests,
                # we need to filter them out before passing to base class
                if is_async and scheduler_output.num_scheduled_tokens:
                    import dataclasses
                    # Create empty scheduler output to match empty model output
            ...
            for req in self.ongoing_prefills:
                # Skip requests that weren't executed in this step
                # (can happen with async scheduling)
            ...
            # With async scheduling, scheduler_output may contain requests that weren't
            # executed yet. Filter to only pass executed requests to the base scheduler.
            if is_async:
                # Filter num_scheduled_tokens to only include executed requests
                ...
                # Create a filtered scheduler output if needed
                if len(filtered_num_scheduled_tokens) != len(scheduler_output.num_scheduled_tokens):
                    # Some requests weren't executed, create filtered output
                    import dataclasses
        ...
        def schedule(self) -> "SchedulerOutput":
            ...
            """
            # For AsyncScheduler: Spyre chunked prefill constraints are bypassed in async mode.
            # The base AsyncScheduler will handle scheduling without Spyre-specific constraints.
            if is_async:
                if not self._async_warning_logged:
                    logger.debug(
                        "Async scheduling is enabled. Spyre chunked prefill constraints "
                        "are bypassed in async mode - using base AsyncScheduler."
                    )
                    self._async_warning_logged = True
                return super().schedule()
            
            # For sync Scheduler: Apply Spyre-specific chunked prefill constraints
            # First purge the full waiting queue into our holdback queue, preserving
            # priority, so that the base scheduler does not see them.
            holdback_queue: deque[Request] = deque()
            while self.waiting:
...
# Export factory functions for use by platform.py
__all__ = [
