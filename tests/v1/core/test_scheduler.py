...
Key behaviours under test:
  ...
  - ChunkedPrefillSpyreScheduler._update_after_schedule() bumps
    ``num_output_placeholders`` by the optimistic prefill-chunk advance so
    update_from_output() can later reconcile the actual committed position
  - ChunkedPrefillSpyreScheduler.update_from_output() reconciles the
    ...
    and decrements ``num_output_placeholders``; under async mode it also
...
class TestChunkedPrefillSpyreSchedulerSchedule:
    ...
    def test_ongoing_prefills_derived_from_running_inflight(self, async_scheduler):
        ...
        but whose advance is still pending (placeholders > 0) is still
        prefilling and SHOULD appear in ongoing_prefills."""
        chunk_size = async_scheduler.chunk_size
        prompt_len = chunk_size + 22  # 150 tokens

        # req_done: fully committed, no placeholders → not prefilling.
        req_done = _make_request("req_done", prompt_len=prompt_len)
        req_done.num_computed_tokens = prompt_len
        req_done.num_output_placeholders = 0

        # req_inflight: optimistically advanced full prefill, but the advance
        # is still in flight (placeholders == prompt_len), so the committed
        ...
        req_inflight.num_output_placeholders = prompt_len
...
class TestUpdateAfterScheduleBumpsPlaceholders:
    """_update_after_schedule bumps num_output_placeholders for prefill chunks."""
    ...
    def test_prefill_chunk_bumps_placeholder(self, sync_scheduler):
        """A prefill-chunk advance is recorded as num_output_placeholders."""
        ...
        # First chunk of a multi-chunk prefill: placeholder bumped to chunk_size.
        assert req.num_computed_tokens == chunk_size
        assert req.num_output_placeholders == chunk_size

    def test_last_prefill_chunk_does_not_bump_placeholder(self, sync_scheduler):
        """The final chunk that completes the prefill does not bump placeholders.
        ...
        should not record a placeholder for it. The final-chunk reconciliation
        is unnecessary because the prefix-cache adjustment only ever fires
        for the first chunk of a prefill.
        """
        chunk_size = sync_scheduler.chunk_size
...
class TestChunkedPrefillSpyreSchedulerUpdateFromOutput:
    ...
    def test_placeholder_cleared_on_commit(self, async_scheduler):
        ...
        placeholder bump made in _update_after_schedule is decremented back."""
        chunk_size = async_scheduler.chunk_size
        prompt_len = chunk_size + 22

        req = _make_request("req1", prompt_len=prompt_len)
        # Simulate the optimistic advance + placeholder bump that
        # _update_after_schedule would have done.
        req.num_computed_tokens = chunk_size
        req.num_output_placeholders = chunk_size
        ...
        # chunk_size; placeholder cleared back to 0.
        assert req.num_computed_tokens == chunk_size
        assert req.num_output_placeholders == 0
    ...
    def test_first_chunk_left_padding_correction(self, async_scheduler):
        ...
        is corrected downward by the padding amount and the placeholder is
        cleared."""
        chunk_size = async_scheduler.chunk_size
